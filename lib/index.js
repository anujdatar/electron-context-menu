...
var buildMenus = function buildMenus(editable) {
  var ctxMenu;
  ...
  if (!editable) {
    ...
    if (window.getSelection().toString() === '') {
      ...
      ctxMenu = new _buildContextMenu.BuildContextMenu(_menuTemplates.templates.reload);
    } else {
      // if text is selected
      ctxMenu = new _buildContextMenu.BuildContextMenu(_menuTemplates.templates.copy);
    }
  } else {
    // if right click in editable textarea
    if (window.getSelection().toString() === '') {
      // no text is selected in editable textarea
      ctxMenu = new _buildContextMenu.BuildContextMenu(_menuTemplates.templates.paste);
    } else {
      // some text is selected in editable textarea
      var selection = window.getSelection().toString();

      if (!_spellchecker.spellChecker.isMispelled(selection)) {
        // selected word is spelled correctly
        ctxMenu = new _buildContextMenu.BuildContextMenu(_menuTemplates.templates.editor);
      } else {
        // selected word is misspelled
        var suggestions = _spellchecker.spellChecker.getCorrectionsForMisspelling(selection);

        if (suggestions.length === 0) {
          // no suggestions foing in dictionary
          ctxMenu = new _buildContextMenu.BuildContextMenu(_menuTemplates.templates.editor, _menuTemplates.templates.noSuggest);
        } else {
          ...
          ctxMenu = new _buildContextMenu.BuildContextMenu(_menuTemplates.templates.editor, suggestedMenu.menu);
        }
      }
    }
  }
